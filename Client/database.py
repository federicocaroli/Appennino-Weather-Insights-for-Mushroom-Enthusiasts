...
"""
This module, `DBMS`, provides a Python interface for interacting with a MariaDB database, specifically designed for managing weather station data. 

It facilitates database connection management, schema creation, and efficient data handling through connection pooling. The module also offers methods for:

- Adding, updating, and retrieving station metadata.
- Storing and querying periodic and instantaneous weather data.
- Performing statistical calculations (e.g., averages, sums, min/max values) on weather records within specified time periods.

The `DatabaseHandler` class ensures robust error handling, logging, and flexibility in database configuration, making it suitable for real-time weather monitoring systems and similar applications.
"""
