...
/**
 * This TypeScript module implements a server-side API for managing a meteorological data dashboard. 
 * It provides endpoints for user authentication, data retrieval, and file access while integrating 
 * with a MariaDB database to store and query weather station data. The module emphasizes secure 
 * and efficient operations, supporting scalable and robust backend services.
 *
 * Key Functionalities:
 *
 * 1. **User Authentication**:
 *    - Implements user login through a `/login` API.
 *    - Issues JWT tokens for authenticated sessions, enabling secure access to restricted resources.
 *    - Supports token renewal via the `/renew` API to maintain session continuity.
 *
 * 2. **Data Retrieval**:
 *    - Fetches weather station metadata (e.g., location, altitude, last update) using the `/get_stations` API.
 *    - Retrieves aggregated meteorological data (e.g., precipitation, temperature, wind speed, humidity) 
 *      for specific stations over customizable time periods via the `/get_data_of_stations` API.
 *
 * 3. **File Access**:
 *    - Serves downloadable resources, such as the exported meteorological data in Excel format, 
 *      through the `/mushroomsDataExport.xlsx` endpoint.
 *
 * 4. **JWT Token Management**:
 *    - Handles token creation, validation, and renewal to ensure secure access control.
 *    - Enforces authentication levels to restrict access to certain APIs based on user roles.
 *
 * 5. **Database Integration**:
 *    - Uses a MariaDB database handler for secure and efficient data storage and retrieval.
 *    - Executes queries for station information and meteorological data aggregation with robust error handling.
 *
 * 6. **Security and Scalability**:
 *    - Supports CORS for cross-origin resource sharing.
 *    - Protects sensitive credentials and JWT secrets, promoting secure operations.
 *    - Utilizes connection pooling for optimized database performance.
 *
 * 7. **Error Handling and Logging**:
 *    - Captures and logs detailed error messages for debugging and operational monitoring.
 *    - Provides meaningful HTTP responses for client-side error handling.
 *
 * This module is designed to serve as a backend for a real-time meteorological dashboard, ensuring 
 * secure data access, scalable performance, and a user-friendly API for managing and visualizing weather data.
 */

const DATABASE_HOST: string = "";
const DATABASE_USER: string = "";
...
function issueJwt(username: string, authLevel: number): string {
	let jwtSecretKey: string = JWT_SECRET_KEY;
...
mushroomServer.get('/mushroomsDataExport.xlsx', parser, async (req: Request, res: Response) => {
	...
	try{
		const EXCEL_FILE_PATH: string = "";

		if (fs.existsSync(EXCEL_FILE_PATH)) {
			res.status(200).sendFile(EXCEL_FILE_PATH);
		} 
		else {
